...
describe("SynchronikLifecycle", () => {
  ...
  it("updates unit status and emits milestone", () => {
    ...
    expect(events.some((e) => e.milestoneId === "unit:w1:started")).toBe(true);
  });
  ...
  it("handles update of unknown unit gracefully", () => {
    expect(() => lifecycle.update("ghost", { status: "error" })).not.toThrow();
  ...
  it("does not emit milestone if unit is missing", () => {
    ...
    expect(events.some((e) => e.milestoneId === "unit:ghost:completed")).toBe(
      true
    ); // still emits, registry not required
  });
